...
router.post("/register", async (req, res) => {
  // Из запроса (req.body) берём данные, которые отправил пользователь: имя, пароль и роль
...
router.post("/login", async (req, res) => {
  ...
  // Пользователь будет использовать этот токен для других запросов (например, чтобы посмотреть книги)
  res.json({ token });
});
